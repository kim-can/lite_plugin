...
inline fun <reified T : UElement> UElement.getLiteParentOfType(strict: Boolean = true): T? = getLiteParentOfType(T::class.java, strict)

@JvmOverloads
fun <T : UElement> UElement.getLiteParentOfType(parentClass: Class<out T>, strict: Boolean = true): T? {
...
private fun getTitle(usages: List<Usage>) = if (usages.isEmpty()) "No lite method call found" else "Lite Method Call"
...
//
//
//        val bg = if (isSelected) UIUtil.getListSelectionForeground(false) else list.background
        panel.foreground = if (isSelected) Color(47,101,202) else list.foreground
...
private class ShowUsagesAction(
    ...
    "Open Find Lite Async",
    "Show all lite async in a separate tool window",
    ToolWindowFind
) {
    init {
...
private class SetBreakpointAction(
    ...
    "Set Breakpoints Lite Async",
    "Set breakpoints at all lite async",
...
private class RemoveBreakpointAction(
    ...
    "Remove Breakpoints Lite Async",
    "Remove breakpoints at all lite async",

...
class CrateFileCodeGenerate(
    ...
    private val layoutDirectory: PsiDirectory?,
    private val filePackageName: String,
    private val xmlPackageName: String,
    ...
    private fun createLiteJavaActivity(activityName: String, liteName: String, xmlModel: XMLModel?) :Boolean {
        ...
        viewProperties.setProperty("PACKAGE_NAME", "$filePackageName;")
        ...
        if(xmlModel != null){
            viewProperties.setProperty("IMPORT_BINDING", "import $xmlPackageName.databinding.${xmlModel.xmlBinding};")
    ...
    private fun createLiteJavaFragment(fragmentName: String, liteName: String, xmlModel: XMLModel?, supportName: String): Boolean {
        ...
        if(xmlModel != null){
            ...
            viewProperties.setProperty("XML_BINDING", "\nprivate val binding: ${xmlModel.xmlBinding} by liteViewBinding()\n")
        }else{
            viewProperties.setProperty("IMPORT_BINDING", "")
            viewProperties.setProperty("IMPORT_BINDING_1", "")
            viewProperties.setProperty("IMPORT_BINDING_2", "")
    ...
    private fun createXML(name: String, typeName: String, activityName: String,type: FileType): XMLModel {
        ...
        if(rootName.isNotBlank()){
            bindingName.append(rootName.checkName("_").checkName("-"))
        }
        ...
        val contentText = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n" +
                ...
                "    tools:context=\"${filePackageName}.${activityName}\">\n" +
                "    <!-- begin to code -->\n" +
                "    \n" +
                "</androidx.constraintlayout.widget.ConstraintLayout>"
        val directory: PsiDirectory = layoutDirectory ?: return  XMLModel(xmlName = xmlFileName.toString(),xmlBinding = bindingName.toString())
        val findFile = directory.findFile(xmlFileName.toString())
        if (findFile != null) {
            return XMLModel(xmlName = xmlFileName.toString(),xmlBinding = bindingName.toString())
        }
        val psiFile  = PsiFileFactory.getInstance(mProject)
    ...
    private fun createLiteKotlinFragment(fragmentName: String, liteName: String, xmlModel: XMLModel?, supportName: String): Boolean {
        ...
        if(xmlModel != null){
            viewProperties.setProperty("IMPORT_BINDING", "import com.boss.android.lite.view.binding.liteViewBinding \n" +
                    "import $xmlPackageName.databinding.${xmlModel.xmlBinding}\n"+
                    "import $xmlPackageName.R")
    ...
    private fun createKotlinLite(parameterClassName: String): Boolean {
        ...
        properties.setProperty("PACKAGE_NAME", filePackageName)
        ...
        if (isFind != null) {
            Messages.showMessageDialog(mProject, "Lite : com.boss.android.lite file existed.", "Lite", Messages.getInformationIcon())
            return false
        }
   ...
   private fun String.checkName(del :String):String{
        val splitList = this.split(del)
        if(splitList.size < 2){
